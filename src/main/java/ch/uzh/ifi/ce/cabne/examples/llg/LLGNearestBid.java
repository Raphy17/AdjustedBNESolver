...
import ch.uzh.ifi.ce.cabne.domains.LLG.LLGSampler2;
import ch.uzh.ifi.ce.cabne.domains.LLG.LLGSamplerBernoulliCorrelated;
import ch.uzh.ifi.ce.cabne.domains.LLG.Proxy;
import ch.uzh.ifi.ce.cabne.domains.LLG.NearestBid;
import ch.uzh.ifi.ce.cabne.domains.LLG.ShapleyWithout;
...
//		context.setRng(3, new CommonRandomGenerator(3));
		...
		//context.setUpdateRule(new UnivariateDampenedUpdateRule(1.0, 1.0, 0.5 / context.getDoubleParameter("epsilon"), true));
		...
		context.setMechanism(new NearestBid());
		
//		no correlation
		context.setSampler(new LLGSampler2(context));
//		with correlation 
//		context.setSampler(new LLGSamplerBernoulliCorrelated(context));
		...
		bneAlgo.setInitialStrategy(2, UnivariatePWLStrategy.makeTruthful(0.5, 1.5));
		
		
//		bneAlgo.setInitialStrategy(1, UnivariatePWLStrategy.makeTruthful(0.0, 0.9));
		...
		BNEAlgorithmCallback<Double, Double> callback = (iteration, type, strategies, epsilon) -> {
			...
			builder.append(String.format("bid %2d", iteration));
...
//			 if not symmetric
//			builder.append("  \n");
//			sPWL = (UnivariatePWLStrategy) strategies.get(1);
//			for (Map.Entry<Double, Double> e : sPWL.getData().entrySet()) {
//				builder.append(String.format("%7.6f",e.getKey()));
//				builder.append(" ");
//				builder.append(String.format("%7.6f",e.getValue()));
