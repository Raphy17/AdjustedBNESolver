...
public class LLGSampler2 extends BidSampler<Double, Double> {

	
	public LLGSampler2(BNESolverContext<Double, Double> context) {
		super(context);
	}
	...
	public Iterator<Sample> conditionalBidIterator(int i, Double v, Double b, List<Strategy<Double, Double>> s) {
		...
		Iterator<Sample> it = new Iterator<Sample>() {
			...
			public Sample next() {
				...
				result[i] = b;
				result[localopponent] = slocal.getBid(localMin + r[0] * (localMax-localMin)); 
