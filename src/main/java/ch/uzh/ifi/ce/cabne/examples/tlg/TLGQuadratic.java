...
import ch.uzh.ifi.ce.cabne.domains.TLG.Quadratic;
import ch.uzh.ifi.ce.cabne.integration.MCIntegrator;
import ch.uzh.ifi.ce.cabne.pointwiseBR.PatternSearch;
import ch.uzh.ifi.ce.cabne.pointwiseBR.UnivariateGridSearch;
...
//		context.setOptimizer(new UnivariateGridSearch(context));		// for BR calculations
...
//		context.setUpdateRule(new UnivariateDampenedUpdateRule(1.0, 1.0, 0.5 / context.getDoubleParameter("epsilon"), true));	//	X

		context.setBRC(new AdaptivePWLBRCalculator(context));
//		context.setBRC(new PWLBRCalculator(context));			// for BR calculations
		...
		bneAlgo.setInitialStrategy(3, UnivariatePWLStrategy.makeTruthful(0.0, 3.0));
