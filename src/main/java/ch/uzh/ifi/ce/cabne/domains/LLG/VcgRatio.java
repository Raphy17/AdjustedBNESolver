...
public class VcgRatio implements Mechanism<Double, Double> {
	...
	public double computeUtility(int i, Double v, Double[] bids) {
		...
		if (i==2) {
            ...
            double[] referencePoint = { Math.max(bids[2] - bids[1], 0), Math.max(bids[2] - bids[0], 0) };
            Double[] vcgPayoff = {bids[0]-referencePoint[0], bids[1]-referencePoint[1]};
        	double totalUtility = bids[0]+ bids[1] - bids[2];
            double payoffFromBid;
            double totalPayoff = vcgPayoff[0]+vcgPayoff[1];
            if (totalPayoff > 0) {
