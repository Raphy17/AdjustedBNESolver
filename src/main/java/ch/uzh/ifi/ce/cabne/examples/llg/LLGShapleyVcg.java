...
import ch.uzh.ifi.ce.cabne.domains.LLG.ShapleyVcg;
...
//		context.setRng(3, new CommonRandomGenerator(3));
		...
		context.setOuterBRC(new AdaptivePWLBRCalculator(context));
		...
		context.setMechanism(new ShapleyVcg());
		//no correlation
...
//		bneAlgo.setInitialStrategy(1, UnivariatePWLStrategy.makeTruthful(0.0, 0.9));
		...
		BNEAlgorithmCallback<Double, Double> callback = (iteration, type, strategies, epsilon) -> {
			...
			builder.append(String.format("svcg %2d", iteration));
