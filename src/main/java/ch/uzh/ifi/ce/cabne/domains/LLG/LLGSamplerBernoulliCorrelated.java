...
public class LLGSamplerBernoulliCorrelated extends BidSampler<Double, Double> {

	
	public LLGSamplerBernoulliCorrelated(BNESolverContext<Double, Double> context) {
		super(context);
	}
	...
	public Iterator<Sample> conditionalBidIterator(int i, Double v, Double b, List<Strategy<Double, Double>> s) {
		...
		double localMin =slocal.getMinValue(); 
		double globalMax = sglobal.getMaxValue();
		double globalMin = sglobal.getMinValue();
		...
		Iterator<Sample> it = new Iterator<Sample>() {
			...
			public Sample next() {
				...
				result[i] = b;
				double value_opponent;
				if (r[2] <= correlation) {
					value_opponent = v;
				} else {
					value_opponent = localMin + r[0] * (localMax-localMin);
				}
				
				result[localopponent] = slocal.getBid(value_opponent); 
				...
				result[2] = globalMin + (globalbound-globalMin) * r[1];
